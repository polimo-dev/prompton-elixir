defmodule PromptOnSDK.SnapshotTest do
  ...
  @updated [:prompton, :use_case_document, :updated]
  @stale [:prompton, :use_case_document, :stale]
  @fetch_error [:prompton, :use_case_document, :fetch_error]
  ...
  describe "boot" do
    test "no snapshot anywhere → not_ready, and boot is not blocked by a slow fetch" do
      ...
      FakeClient.set(:fetch_use_cases, fn _etag, _opts ->
      ...
      assert %{source: :none, stale?: true} = PromptOnSDK.use_case_document_info()
    ...
    test "loads disk cache with sidecar (source :disk) then promotes to :remote on 304" do
      ...
      FakeClient.set(:fetch_use_cases, fn ~s("e1"), _opts -> {:ok, %{status: 304}} end)
      start_sdk(disk_cache: path)

      # Loaded synchronously in init, so use_case works immediately
      assert {:ok, r} = PromptOnSDK.use_case("diary_generation", %{language: "ko", plan: "pro"})
      assert r.source in [:disk, :remote]
      assert r.etag == ~s("e1")

      eventually(fn -> PromptOnSDK.use_case_document_info().source == :remote end)
      info = PromptOnSDK.use_case_document_info()
      ...
      assert [{:fetch_use_cases, ~s("e1"), [receive_timeout: 3_000]}] =
               FakeClient.calls(:fetch_use_cases)
    ...
    test "falls back to bundle when disk cache is missing; failure keeps :bundle and emits stale" do
      ...
      FakeClient.set(:fetch_use_cases, fn _etag, _opts -> {:error, :timeout} end)

      start_sdk(disk_cache: tmp_path("missing.json"), bundle: {:file, bundle})

      assert {:ok, %{source: :bundle}} = PromptOnSDK.use_case("diary_generation")
      ...
      assert %{source: :bundle, stale?: true} = PromptOnSDK.use_case_document_info()
    end
    ...
    test "rejects disk/bundle snapshot whose environment does not match the configured one" do
      ...
      FakeClient.set(:fetch_use_cases, fn _etag, _opts -> {:error, :nxdomain} end)
    ...
    test "a staging app loads a staging snapshot (the environment is configured, not derived)" do
      ...
      assert {:ok, %{source: :disk}} = PromptOnSDK.use_case("diary_generation")
      assert FakeClient.calls() == []
      assert PromptOnSDK.refresh_use_case_document() == :ok
    end
  ...
  describe "remote fetch" do
    test "200 → persistent_term replaced, disk cache + sidecar written, updated telemetry" do
      ...
      FakeClient.set(:fetch_use_cases, fn nil, _opts ->
      ...
      assert {:ok, %{source: :remote, etag: ~s("e2")}} =
               PromptOnSDK.use_case("diary_generation")
    ...
    test "polling sends If-None-Match; 304 leaves snapshot unchanged" do
      ...
      FakeClient.set(:fetch_use_cases, fn
        nil, _ ->
          ok_200(body, ~s("e3"))
      ...
      assert %{etag: ~s("e3"), source: :remote, stale?: false} =
               PromptOnSDK.use_case_document_info()
    end
    ...
    test "failure after success marks stale (keeps old snapshot) and backs off; recovery clears stale" do
      ...
      FakeClient.set(:fetch_use_cases, fn etag, _ ->
      ...
      assert {:ok, %{source: :remote}} = PromptOnSDK.use_case("diary_generation")
      assert %{stale?: true} = PromptOnSDK.use_case_document_info()
      ...
      eventually(fn -> PromptOnSDK.use_case_document_info().stale? == false end, 1_500)
    end

    test "invalid body is a fetch error, snapshot not replaced" do
      attach_telemetry([@fetch_error])
      FakeClient.set(:fetch_use_cases, fn _, _ -> ok_200(~s({"schema_version": "x"})) end)
      start_sdk()

      assert_receive {:telemetry, @fetch_error, _,
                      %{reason: {:decode, {:invalid_use_case_document, _}}}},
                     500
    ...
    test "client exceptions are contained" do
      attach_telemetry([@fetch_error])
      FakeClient.set(:fetch_use_cases, fn _, _ -> raise "kaboom" end)
      start_sdk()
    ...
    test "refresh/0 fetches synchronously" do
      body = snapshot_json()
      FakeClient.set(:fetch_use_cases, fn _, _ -> {:error, :econnrefused} end)
      start_sdk()
      eventually(fn -> FakeClient.calls(:fetch_use_cases) != [] end)
      assert PromptOnSDK.use_case("diary_generation") == {:error, :not_ready}

      FakeClient.set(:fetch_use_cases, fn _, _ -> ok_200(body, ~s("e5")) end)
      assert PromptOnSDK.refresh_use_case_document() == :ok
      assert {:ok, %{etag: ~s("e5")}} = PromptOnSDK.use_case("diary_generation")

      FakeClient.set(:fetch_use_cases, fn _, _ -> {:error, :boom} end)
      assert PromptOnSDK.refresh_use_case_document() == {:error, :boom}
    end

    test "map bodies are accepted and re-encoded for disk" do
      path = tmp_path("cache.json")
      FakeClient.set(:fetch_use_cases, fn _, _ -> ok_200(Fixtures.snapshot(), ~s("e6")) end)
      start_sdk(disk_cache: path)
      eventually(fn -> File.exists?(path) end)
      assert {:ok, _, _} = PromptOnSDK.UseCaseDocument.decode_json(File.read!(path))
    end
  end
  ...
  describe "modes" do
    test ":test mode never touches the client and refresh is a no-op" do
      ...
      assert FakeClient.calls() == []
      assert PromptOnSDK.refresh_use_case_document() == :ok
      assert PromptOnSDK.use_case("x", %{}) == {:error, :not_ready}
    end
    ...
    test "live without api_key/base_url runs on local snapshot only" do
      ...
      assert FakeClient.calls() == []
      assert {:ok, %{source: :bundle}} = PromptOnSDK.use_case("diary_generation")
      assert PromptOnSDK.refresh_use_case_document() == {:error, :remote_disabled}
    end
  end
