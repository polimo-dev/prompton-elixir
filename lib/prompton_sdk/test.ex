defmodule PromptOnSDK.Test do
  ...
  * There is no HTTP at all: the prompt document loader is idle, and `PromptOnSDK.log/1` /
  ...
  * Documents are injected with `put_prompt_document/1` (map, JSON file, or
    `PromptOnSDK.PromptDocument.t()`) or
    `stub/2` (a minimal entry for one Prompt).
  ...
  Named prompts (language branches) are stubbed with `template: `:
  `stub("support_reply", %{model: ..., template: "ko", ...})` corresponds to
  `PromptOnSDK.prompt("support_reply", template: "ko")`. Calling it several times for the same key with
  different names accumulates pins.
      ...
      test "worker records a log" do
        ...
        assert_logged(%{"prompt_key" => "support_reply", "status" => "ok"})
      end

  `stub/2` **accumulates** onto the existing test document (multiple Prompts). `put_prompt_document/1`
  ...
  (default `:chat`; `:embedding` has no prompt), `template: ` (the template name, default
  ...
  @doc """
  Injects a prompt document: a `map` (§6.2 format, string/atom keys) | `{:file, path}` (JSON) |
  `PromptOnSDK.PromptDocument.t()`. `source: :manual`, `etag: "test"`. Raises on a decode
  failure.
  """
  @spec put_prompt_document(map() | {:file, String.t()} | PromptDocument.t(), keyword()) :: :ok
  def put_prompt_document(document, opts \\ [])

  def put_prompt_document({:file, path}, opts) do
    case PromptDocument.decode_json(File.read!(path)) do
      {:ok, data, _warnings} ->
        put_prompt_document(data, opts)

      {:error, reason} ->
        raise ArgumentError, "invalid prompt document file #{path}: #{inspect(reason)}"
    end
  end

  def put_prompt_document(%PromptDocument{} = data, opts) do
    Store.put(
  ...
  def put_prompt_document(map, opts) when is_map(map) do
    case PromptDocument.decode(map) do
      {:ok, data, _warnings} -> put_prompt_document(data, opts)
      {:error, reason} -> raise ArgumentError, "invalid prompt document: #{inspect(reason)}"
    end
  end

  @doc """
  Builds a minimal prompt document entry for one Prompt and merges it into the current test document
  (creating one if there is none). One Deployment pin → one PromptVersion/Model. Calling it again
  for the same prompt with only `template: ` changed **adds** a pin under that name (the model and
  params of the last call win).
  """
  @spec stub(String.t() | atom(), map() | keyword()) :: :ok
  def stub(prompt_key, spec) do
    key = to_string(prompt_key)
    spec = Map.new(spec)
    kind = Map.get(spec, :kind, :chat)
    model = Map.get(spec, :model) || raise ArgumentError, "stub/2 requires :model"
    template = to_string(Map.get(spec, :template, "default"))

    ids = %{pv: "stub-pv-#{key}-#{template}", model: "stub-model-#{key}"}
    prompt_version = stub_prompt_version(kind, ids.pv, spec)

    base =
      case Store.get() do
        %{source: :manual, data: %PromptDocument{} = data} -> data
        _ -> %PromptDocument{environment: Map.get(spec, :environment)}
      end

    pins =
      case Map.get(base.deployments, key) do
        %{template_pins: pins} when is_map(pins) -> pins
        _ -> %{}
      end

    pins = if prompt_version, do: Map.put(pins, template, ids.pv), else: %{}

    {:ok, uc_data, _} =
      PromptDocument.decode(%{
        "schema_version" => PromptDocument.schema_version(),
        "prompts" => %{key => stub_prompt(key, kind, spec)},
    ...
    merged = %PromptDocument{
      base
      | prompts: Map.merge(base.prompts, uc_data.prompts),
    ...
    merged = %PromptDocument{
      merged
      | prompts:
          Map.new(merged.prompts, fn {k, uc} ->
            {k, %{uc | deployment: Map.get(merged.deployments, k)}}
          end)
    }

    put_prompt_document(merged, [])
  end
  ...
  defp stub_prompt(key, kind, spec) do
    %{
      "id" => "stub-uc-#{key}",
      "kind" => to_string(kind),
  ...
  @doc "Clears the test document (`PromptOnSDK.prompt/2` → `{:error, :not_ready}`)."
  @spec clear() :: :ok
  def clear do
    Store.erase()
    :ok
  ...
  payload policy is applied). Returns the matched gen.
      ...
      gen = assert_logged(%{"prompt_key" => "support_reply"})
