...
reproduces the expected values byte for byte. When two SDKs disagree about how a template renders,
...
(`PromptDocument`, `Template`, `StopKind`) directly, so these fixtures also describe the
...
| `prompt.json` | Prompt document + prompt (+ template name) → model, params, template version, filled messages | 3 documents, 15 cases |
...
the template version is committed, so a template that fails lint can never reach a prompt document.
...
`documents[document_ref]`, select `prompt` with the optional `prompt` name, and — when
...
```
params            = prompt.default_params  <- deployment.params
...
A deployment revision is a **pin**, not a router. It is one model plus one pinned template version
per template name. At request time the only selection axis is the template name (default `"default"`),
...
* Prompt of kind `embedding` has no template at all: `prompt` and `prompt_version` are `null`, and
  a template name passed with the request is ignored rather than rejected.
* If the prompt document references a template version or model id it does not contain, selection
  still succeeds with those fields `null` and a warning. A healthy server never emits such a
  document.
